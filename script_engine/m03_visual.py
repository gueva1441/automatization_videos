...
- foto_madre_ref: the recurring documented object this image DEPICTS as its focus, if any. The visual canon above names the central subject and any documented objects. Set this to that object's canonical name when THIS beat's focus IS the recurring object — even when the narration refers to it by metaphor, function or material rather than plainly (its identity is the same object). Use the exact string "__subject__" for the central recurring subject, or the exact name shown after "objeto:" in OBJETOS DOCUMENTADOS for a documented object. Default "": leave EMPTY unless the object is the actual focus of THIS image — an object merely mentioned, alluded to, or sitting in the background is "" (its form is already held by the prose slots). ONE ref per item: if two documented objects share a beat, pick the single primary focus and leave the rest to the prose. This slot is routing only — it is NOT rendered as text.
