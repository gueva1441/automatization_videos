...
| **m03_visual** `script_engine` | **Genera image_prompts EN con anchor exacto por imagen** (two-step), dispatch por motor: Flux (prosa+ancla) / Kling (prosa+tail-bake) / **Seedream (eslabón 3b: SKELETON de slots → FLUIDIFICADOR Pro per-item teje prosa por la fórmula del perfil 3a → GUARDA 1 candado de cifras por significado + post-check determinista). Lee el canon 2-capas del eslabón 2; R3 invertida (text_in_image rótulo permitido). Path flux Y veo migrados a seedream: el cap veo (1 foto Veo + N stills DepthFlow) pasa TODAS las fotos por el skeleton vía plan sintético [veo_anchor]+supps; el video_prompt (motion) sale de una call Pro dedicada (`_render_prompts_seedream_veo`)** | `assign_visual_prompts()` |
