...
| 2026-07-04 | 134d | **RUTA B — foto madre de ESTABLECIMIENTO para kind=place (3 textos de prompt).** (1) `foto_madre._prompt_place` NUEVA plantilla: retrato del establecimiento (vista aérea/establishing 3/4, footprint + silueta completos) desde distinctive_features/scale/materials/color/era + estado de condition_evolution + anachronism_blocklist. **Época-DEL-SUJETO, NUNCA época-de-la-foto** (lección 133): dice "wide, full-color establishing view", ata la década a la ARQUITECTURA ("period-correct 1970s architecture"), NADA de "archival photograph from [era]" que fuerza B&N/placa — la plantilla nace sin ese bug (el fix de la plantilla object sigue gateado aparte). (2) m03 slot `foto_madre_ref` gana cláusula PLACES: `__subject__` SOLO en beats establishing/exterior del complejo; interiores/rooms/corredores/detalle → EMPTY (su identidad no es la silueta del edificio). Few-shot conceptual. **+ filtro DETERMINÍSTICO** `_apply_place_exterior_filter` (red de seguridad del rule: subject_ref ∈ {establishing_shot, main_subject} y shot_scale≠detail → ancla; resto → strip). (3) CHASCADA 2: sacada la regla muerta "CONJUNTO vs PIEZA (varias partes → 1 entrada)" del BLOQUE 4 del 4e (†131, redundante con el criterio de RECURRENCIA; Pattern #91). La taxonomía kind del 4e queda INTACTA (place sigue place). R1 (estado por beat) se hereda sin código nuevo. Lint: `test_module_134d_place.py`. | Evidencia visual topic ba6c3cac (Prisión Orleans/Katrina): 4 establishing shots = 4 prisiones DISTINTAS (la torre aparece/desaparece). 4e clasificó "place" (defendible: campus de 12 edificios) → `foto_madre` solo anclaba kind=="object" → candidatos vacíos → registry {} → 80/80 t2i → Seedream inventa otra prisión por render. El nicho raíz "abandonados" cae casi entero en kind=place → clase, no caso. |
| 2026-07-04 | 134 | **m03 (path seedream) — REGLA 3 (text-leakage) pasa de FATAL-de-un-tiro a retry+feedback → backstop scrub (espejo de Guarda 1).** Antes `_validate_no_text_leakage` levantaba a la primera fuga → 1 imagen de 95 mataba el topic entero (y todo el gasto Pro ya hecho). Ahora, en `_fluidify_with_leakage_guard`: teje → scrub_documented_names → `_find_text_leakage` (detección INTACTA: mismos `TEXT_LEAKAGE_PATTERNS`, misma Pasada 1/2, camino-C exime el rótulo sancionado de `text_in_image`) → si fuga y quedan re-tejidos (máx 2): re-teje SOLO ese ítem con **feedback** (el fragmento que fugó + instrucción POSITIVA "describe the OBJECT plainly, a surface simply worn/blank/bare"; los ejemplos prohibidos van como plantilla conceptual 'the word ...', no como few-shot copiable, AP3) → si sigue fugando: **backstop** scrub quirúrgico de la cláusula (expande a attach-word/coma/oración, sin inglés roto) + WARN hermano de Guarda 1 (run SIGUE) → si tras scrub queda < `PROMPT_MIN_CHARS`: recién ahí `VisualValidationError` (terminal legítimo). Presupuesto de leakage INDEPENDIENTE del de Guarda 1. Kling/Flux (`allow_intentional_text=False`) y `_validate_no_text_leakage` NO se tocan. + yapa `fase1_5`: rótulo FAIL del paso REAL (`_derive_failed_step`), no el `from_step` mentiroso. Lint: `test_module_03_leakage_retry.py`. | Corrida real 9e899474 (2026-07-04): m03 completó 7 caps (~$2.69 Pro, 143 calls) y **1 imagen** (cap 4 img #14) tejió `the word 'streptomycin'` → crash fatal → topic muerto, todo el gasto a la basura. Asimetría de diseño: Guarda 1 (cifras) degradaba y seguía; regla 3 mataba de un tiro. Clase, no caso: cualquier tema con drogas/carteles/documentos nombrados lo dispara. |
...
| 2026-07-04 | 137 | **LEY 1 "PINTOR-NO-ACTOR" (restauración de la regla 6 + cuerpo-por-situación) en m03 seedream + m09 thumbs.** El port de la doctrina Kling→seedream (117+) TIRÓ la regla 6 (traducción física de metáforas) y el slot `mood` pedía "emotional tone" (abstracción). Además no existía regla de estado físico de PERSONAS (STATE BY BEAT solo gobernaba al sujeto-objeto). Fix: bloque PHYSICAL TRANSLATION (R6) + BODY CARRIES THE SITUATION en `SYSTEM_INSTRUCTION_VISUAL_SEEDREAM`; slot `mood` reescrito a física-solamente; misma ley en `_HERO_SYSTEM` de m09. ⚠ CLASE DOC-DRIFT: el mismo port también MUTÓ una cita ("cara al frente" de la regla 1-etnia quedó firmada como R8) — pendiente del frente LEY 2/mirada. Regla de la casa nueva: todo port de doctrina a un prompt se audita regla-por-regla contra el MPR. | Ojo de Omar en el MP4 de Orleans (ba6c3cac): múltiples figuras con el agua al pecho, caras limpias/secas/serenas "como para una cita". Evidencia 136: ch03_17 (triple emoción abstracta + cara seca tras días en agua de cloaca) vs ch05_10 (pidió física → renderizó actuado). |
